...
Array.prototype.getObjectByMinValue = function (propName) {
    if (this.length == 0) {
        return null;
    }
    var minObject = this[0];
    var minVal = this[0][propName];
    for (var i = 1; i < this.length; i++) {
        if (this[i][propName] < minVal) {
            minObject = this[i];
            minVal = this[i][propName];
        }
    }
    return minObject;
};
Array.prototype.getObjectByMaxValue = function (propName) {
    if (this.length == 0) {
        return null;
    }
    var maxObject = this[0];
    var maxVal = this[0][propName];
    for (var i = 1; i < this.length; i++) {
        if (this[i][propName] > maxVal) {
            maxObject = this[i];
            maxVal = this[i][propName];
        }
    }
    return maxObject;
};
Array.prototype.getObjectByMaxValueSmallOrEqualThan = function (propName, val) {
    if (this.length == 0) {
        return null;
    }
    var maxObject = null;
    ...
    for (var i = 0; i < this.length; i++) {
        ...
        } else if (this[i][propName] <= val && this[i][propName] > maxVal) {
            maxObject = this[i];
            maxVal = this[i][propName];
        }
    }
...
Array.prototype.getObjectByMaxValueSmallThan = function (propName, val) {
    if (this.length == 0) {
        return null;
    }
    var maxObject = null;
