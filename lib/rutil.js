...
Date.prototype.addYears = function (years) {
    ...
    var newDT = new Date(this.getFullYear() + years, this.getMonth(), this.getDate());
    //newDT当月天数不够，会导致newDT为下一月，date设为0回到上月最后一天
    if (newDT.getMonth() != this.getMonth()) {
        newDT = new Date(newDT.getFullYear(), newDT.getMonth(),0);
    }
    return newDT;
}
Date.prototype.addMonths = function (months) {
    months = parseInt(months);
    var newDT = new Date(this.getFullYear(), this.getMonth() + months, this.getDate());
    if (newDT.getMonth() != (this.getMonth() + months) % 12) {
        newDT = new Date(newDT.getFullYear(), newDT.getMonth(), 0);
    }
    return newDT;
}
Date.prototype.addDays = function (days) {
    ...
    var _t = this.getTime();
    var _now = new Date();
    var newDT = new Date(_now.getFullYear(), _now.getMonth(), _now.getDate() + days);
    return newDT;
}
...
String.prototype.getBirthdate = function () {
    if (/^(\d{18,18}|\d{17,17}(x|X))$/.test(this)) {
        var bd = this.substring(6, 4) + "-" + this.substring(10, 2) + "-" + this.substring(12, 2);
        try {
            var d = new Date(bd);
            return db;
        } catch (e) {
...
String.prototype.getAge = function () {
    if (/^(\d{18,18}|\d{17,17}(x|X))$/.test(this)) {
        ...
        var _age = myDate.getFullYear() - parseInt(idCard.substring(6, 10)) - 1;
