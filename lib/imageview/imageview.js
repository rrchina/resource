//modified at:2022-12-16 modelSize可按百分比；部分样式修改；修正窗口缩放时位置计算；切换图片时重置图片旋转;esc退出
//<a href="javascript:void(0)" data-magnify="gallery2" data-group="g2" data-src="./images/5.png" data-caption="测试图片5">
    ...
    * */
    ...
    magnify.prototype = {
        ...
        open: function () {
            ...
            this.$magnify.css({ width: this.settings.modalSize[0], height: this.settings.modalSize[1] })
        ...
        setModalSize: function () {
            ...
            var newLeft = (winWidth - $modal.outerWidth()) / 2;
            var newTop = (winHeight - $modal.outerHeight()) / 2;
            $modal.css({
                left: newLeft,
                top: newTop
            });
        ...
        setModalPos: function (modal) {
            var winWidth = $W.width(),
                ...
                modalWidth = modal.outerWidth(),//modalSize[0],
                modalHeight = modal.outerHeight();//modalSize[1];
            modal.css({
                width: modalSize[0],
                height: modalSize[1],
        ...
        loadImg: function (imgSrc, tag) {
            ...
            //imgSrc = '/lib/imageview/images/b1.jpg';
        ...
        transformDeg: function (currentImg, currentDeg) {
            ...
            if (useIeTransforms) {
                ...
                for (var j = 0; j < _IeStyle.length; j++) {
                    ...
                    if (this.$magnify.outerWidth() >= imgHeightNum) {
                        //缩小了 或者没变
                        currentImg.css(IEprefix + 'left', this.$magnify.outerWidth() / 2 - imgWidthNum / 2 + 'px');
                        currentImg.css(IEprefix + 'top', this.$magnify.outerHeight() / 2 - imgHeightNum / 2 + 'px');
                    } else {
                        //放大了
                        currentImg.css(IEprefix + 'left', -(imgWidthNum - this.$magnify.outerWidth()) / 2 + 'px')
                        currentImg.css(IEprefix + 'top', -(imgHeightNum - this.$magnify.outerHeight()) / 2 + 'px')
                    }
                }
            }
        },
